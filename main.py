...
def build_set():
    ...
    blacklist = [x.strip() for x in blacklist_input.split(",") if x.strip()] if blacklist_input else []
    ...
    for index, (score, gear) in enumerate(best_sets, start=1):
        ...
        for item in gear.values():
            output_lines.append(f"{item['name']} (i{item['ilvl']})")
        output_lines.append("")

    messagebox.showinfo("Best Gear Sets", "\n".join(output_lines))
...
def start_gui():
    ...
    frame = tk.Frame(root)
    frame.pack(pady=20)
    tk.Button(frame, text="Detect Highest iLvl", command=detect_ilvl, width=28).pack(pady=5)
    tk.Button(frame, text="Build Best Sets", command=build_set, width=28).pack(pady=5)
    log("===================================")
