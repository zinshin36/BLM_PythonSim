...
def build_set():
    ...
    current_gear_pool = fetch_gear_range(min_ilvl, current_max_ilvl, use_cache=True)

    if not current_gear_pool:
        messagebox.showerror("Error", "No gear found — try clearing cache or check API.")
        return
    ...
    blacklist = [x.strip() for x in blacklist_input.split(",")] if blacklist_input else []

    best = build_best_set(current_gear_pool, blacklist)
    optimized = apply_materia_logic(best)

    if not optimized:
        messagebox.showerror("Error", "No gear found after optimization.")
        return

    output = "\n".join(
        f"{item['name']} (i{item['ilvl']}) - Materia Slots: {item['materia_slots']}"
